...
import { CATEGORICAL_COLORS, CHART_COLORS, useResize, formatDescription } from "../../../utils";
...
export const BattingOutcomeChart: React.FC<Props> = ({ pitches }) => {
  ...
  const data = useMemo(() => {
    ...
    pitches.forEach(p => {
      const desc = p.description?.replace(/_/g, " ") || "unknown";
      const label = desc.split(" ").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
      if (p.events) {
        const eventLabel = formatDescription(p.events);
        counts[eventLabel] = (counts[eventLabel] || 0) + 1;
      } else {
    ...
    g.selectAll("path")
      ...
      .on("mouseover", function (_event, d) {
        ...
        tooltip.style("display", "block")
          .html(`<strong>${d.data.name}</strong>: ${d.data.count}`);
      })
